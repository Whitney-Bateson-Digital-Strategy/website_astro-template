function deepMerge<T>(target: Record<string, unknown>, ...sources: Record<string, unknown>[]): T {
  const result = { ...target };
...
const merge = <T>(target: Record<string, unknown>, ...sources: Record<string, unknown>[]) => deepMerge<T>(target, ...sources);
